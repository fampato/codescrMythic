...
local library = {
	...
	colors = {
		main = Color3.fromRGB(68, 202, 186),
...
local foldername = "Mythic Hub Premium Scripts"
...
function Check()
	...
	if World1 then
        ...
        if  Farmlevel  and (CFrameQuest.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > 1200 then
            ByPass(CFrameQuest)
        end
    ...
    elseif MtLevel == 30 or MyLevel <= 39 then
        Name = "Pirate [Lv. 35]"
...
if ThreeWorld then
    ...
    elseif Mylevel == 2400 or MyLevel <= 2424 then
        ...
        CFrameMon = CFrame.new(-1353.02161, 72.0601883, -14570.6445, -0.999970913, 1.99122461e-08, -0.00762624247, 1.99347028e-08, 1, -2.86847213e-09, 0.00762624247, -3.02041547e-09, -0.999970913)
        CFrameQuest = CFrame.new(-1144.60767, 14.1330481, -14445.1045, 0.0662112832, 4.61361189e-08, 0.997805655, 4.31780443e-08, 1, -4.91027414e-08, -0.997805655, 4.63344492e-08, 0.0662112832)
    elseif  MyLevel >= 2425 then
        ...
        CFrameMon = CFrame.new(-831.841919, 88.7651443, -14276.1064, 0.962894142, 7.71796707e-08, 0.26987946, -7.82888563e-08, 1, -6.65406574e-09, -0.26987946, -1.47213921e-08, 0.962894142)
        CFrameQuest = CFrame.new(-1144.60767, 14.1330481, -14445.1045, 0.0662112832, 4.61361189e-08, 0.997805655, 4.31780443e-08, 1, -4.91027414e-08, -0.997805655, 4.63344492e-08, 0.0662112832)
       end
    end
end
spawn(function()
    while game:GetService("RunService").Stepped:wait() do
		pcall(function()
        	if Farmlevel or AutoKatakuri  then
...
spawn(function()
    while wait(0.1) do
        pcall(function()
            if _G.Clip or  Farmlevel  or getgenv().autonewworld or AutoKatakuri then
...
local Mauto = Page:CreateSection({
    Name = "Main", -- ชื่อ
    Side = "Left" -- ตำแหน่ง Left/Right
})

local SettingFarm = Page:CreateSection({
    Name = "Weapon / Eqipe", -- ชื่อ
...
Mauto:AddToggle({
	...
	Value = _G.Settings.Farmlevel, -- ปรับค่าToggle true/false or Config
    Callback = function(value)
		Farmlevel  = value
		getgenv().autonewworld = value
		_G.Settings.Farmlevel = value
		Farmlevelfast = value
		saveSettings()
    end
})
...
spawn(function()
	game:GetService("RunService").Heartbeat:Connect(function() 			Check()
		pcall(function()
			Check()
			for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
				if Farmlevel and Startlevel and v.Name == Name and (v.HumanoidRootPart.Position - levelpos.Position).magnitude <= 350 then
					v.HumanoidRootPart.CFrame = levelpos
    ...
    while wait() do
        if Farmlevel then
            ...
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == false then
                Startlevel = false
                 Distance = (          CFrameQuest .Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 14
                wait(1.2)
                TP(        CFrameQuest)
                game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("StartQuest", QuestName,lvquest)
                ...
                for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                    if  v.Name == Name and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
                            repeat wait(_G.Fast_Delay)
                                ...
                                TP(v.HumanoidRootPart.CFrame * CFrame.new(0,_G.Y,0))
                                v.HumanoidRootPart.Size = Vector3.new(60,60,60)
                                v.HumanoidRootPart.CanCollide = false
								 levelpos = v.HumanoidRootPart.CFrame
                                Startlevel = true
                            until  Farmlevel == false or v.Humanoid.Health <= 0 or game.Players.LocalPlayer.PlayerGui.Main.Quest.Visible == false
                        end
                    end
                end
            end
...
SettingFarm:AddDropdown({
	Name = "Select Weapon",
	Value = _G.Settings.SelectWeapon, -- ค่าที่จะให้มันเลือก
	List = WeaponList,
	Callback = function(value)
		SelectWeapon = value
...
SettingFarm:AddToggle({
    Name = "Fast Ins",
	Value = _G.Settings.fastnocd, -- ปรับค่าToggle true/false or Config
    Callback = function(value)
		fastnocd = value
		_G.Settings.fastnocd = value
    end
})
spawn(function()
	while wait() do
		pcall(function()
			if fastnocd then
				repeat wait(_G.Fast_Delay)
					AttackNoCD()
				until not  _G.Settings.fastnocd
			end
		end)
	end
end)
local  CakeCheck = Event:AddLabel({
    Name = ""
})
spawn(function()
	while wait() do
		pcall(function()
			if not game.ReplicatedStorage:FindFirstChild("Cake Prince [Lv. 2300] [Raid Boss]")then	
				CakeCheck:Set("Boss Not Spawn :  ❌")
			else   CakeCheck:Set("Boss Spawn :  ✅")
			end
		end)
	end
end)
local Mob_Kill_Cake_Prince = Event:AddLabel({
	Name = "",
})
	...
	while wait() do
		pcall(function()
			...
			elseif string.len(game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CakePrinceSpawner")) == 86 then
				...
				Mob_Kill_Cake_Prince:Set("Kill : Boss")
			end
		end)
	end
end)
spawn(function()
	game:GetService("RunService").Heartbeat:Connect(function()
		pcall(function()
			for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
				if AutoKatakuri and StartCakeMagnet and (v.Name == "Cookie Crafter [Lv. 2200]" or v.Name == "Cake Guard [Lv. 2225]" or v.Name == "Baking Staff [Lv. 2250]" or v.Name == "Head Baker [Lv. 2275]") and (v.HumanoidRootPart.Position - PosMonCake.Position).magnitude <= 350 then
...
Event:AddToggle({
	...
	Value = 0, -- ปรับค่าToggle true/false or Config
    Callback = function(value)
		AutoKatakuri = value
    end
})
						...
						for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do 
							if v.Name == "Cake Prince [Lv. 2300] [Raid Boss]" then
								repeat wait()
									...
									v.HumanoidRootPart.Size = Vector3.new(60,60,60)
									v.HumanoidRootPart.CanCollide = false
								until AutoKatakuri == false or not v.Parent or v.Humanoid.Health <= 0
							end
						end
					else
						TP(CFrame.new(-2159.60327, 69.9830475, -12398.5469, 0.99892211, -1.59716163e-09, 0.0464178175, -2.51538967e-09, 1, 8.85401406e-08, -0.0464178175, -8.85614639e-08, 0.99892211))
					end
				else
					if game.Workspace.Enemies:FindFirstChild("Baking Staff [Lv. 2250]") or game.Workspace.Enemies:FindFirstChild("Head Baker [Lv. 2275]") or game.Workspace.Enemies:FindFirstChild("Cake Guard [Lv. 2225]") or game.Workspace.Enemies:FindFirstChild("Cookie Crafter [Lv. 2200]")  then
						for i,v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do  
							if (v.Name == "Baking Staff [Lv. 2250]" or v.Name == "Head Baker [Lv. 2275]" or v.Name == "Cake Guard [Lv. 2225]" or v.Name == "Cookie Crafter [Lv. 2200]") and v.Humanoid.Health > 0 then
								...
								until AutoKatakuri == false or not v.Parent or v.Humanoid.Health <= 0 or game:GetService("ReplicatedStorage"):FindFirstChild("Cake Prince [Lv. 2300] [Raid Boss]") 
							end
							end
						else
